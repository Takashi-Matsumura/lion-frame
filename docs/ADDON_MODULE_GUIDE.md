...
| `developer` | `app/(menus)/(developer)/<path>/` |
...
`menuGroup: "developer"` を指定すると、開発環境（`NODE_ENV === "development"`）でのみ表示されるメニューを作成できます。本番環境では自動的に非表示になります。

```typescript
menus: [
  {
    id: "nfc-registration",
    menuGroup: "developer",       // ← 開発環境専用
    requiredRoles: ["ADMIN"],     // ← ADMIN のみアクセス可
...
- ページ配置先: `app/(menus)/(developer)/<path>/`
- `layout.tsx` の `NODE_ENV` チェックでフィルタリング
- `middleware.ts` でのルートガードも必要

---

## 実装例: NFCカードモジュール

DB・API・WebUSB連携を含む実装済みアドオンモジュールです。

### ファイル構成

```
lib/addon-modules/nfc-card/
├── module.tsx            # モジュール定義（developer セクション）
├── index.ts              # re-export
├── nfc-card-service.ts   # サービスレイヤー（Prisma CRUD）
└── nfc-reader.ts         # WebUSB NFC リーダー通信（RC-S300）

app/(menus)/(developer)/nfc-registration/
├── page.tsx              # Server Component
├── NfcRegistrationClient.tsx  # Client Component
└── translations.ts       # 翻訳（en/ja）

app/api/nfc-card/
├── route.ts              # GET（一覧）, POST（登録）
├── [id]/route.ts         # PATCH（無効化）
└── employee/[employeeId]/route.ts  # GET（社員検索）
```
...
| 項目 | 内容 |
|------|------|
| WebUSB | Sony RC-S300/RC-S380 (PaSoRi) FeliCa リーダー対応 |
| カード管理 | 1社員1有効カード、論理削除（履歴保持）、再登録対応 |
| セクション | `developer`（開発環境のみ表示） |
| 対象ロール | ADMIN |
...
- [ワークフローモジュール（UIモック）](../apps/web/lib/addon-modules/workflow/)
- [NFCカードモジュール（実装済み）](../apps/web/lib/addon-modules/nfc-card/)
