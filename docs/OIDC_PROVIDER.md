...
社内で新しいアプリを作るたびに認証を個別実装するのをやめ、LionFrame のアカウント・パスキー MFA・RBAC を OIDC 経由で共通化します。
...
運用負荷（別プロセス・別 DB・別ログ基盤）が大きく、社内用途には過剰。また LionFrame のパスキー MFA・RBAC をそのまま使うには結局 LionFrame を Identity Source として繋ぎ込む必要があり、メリットが薄い。
...
- カスタムクレーム `lion:role` / `lion:mfa_used` のマッピング（`lion:mfa_used` は ID Token 専用）
...
4. ログインフォーム（`CredentialsLoginForm` / `PasskeySignInButton`）は、成功時に `callbackUrl` クエリへ遷移する（`lib/services/safe-redirect.ts` で相対パスのみ許可）。
