export const aiChatTranslations = {
  en: {
    title: "AI Chat",
    subtitle: "Chat with AI assistant",
    placeholder: "Type your message...",
    send: "Send",
    thinking: "AI is thinking...",
    newChat: "New Chat",
    clearChat: "Clear Chat",
    clearConfirm: "Are you sure you want to clear the chat?",
    emptyState: "Start a conversation with AI",
    emptyStateHint: "Type a message below to begin",
    errorTitle: "Error",
    errorMessage: "Failed to get response from AI. Please try again.",
    aiDisabled: "AI is not enabled",
    aiDisabledHint: "Please enable AI in System Settings > AI Settings",
    you: "You",
    ai: "AI",
    copied: "Copied to clipboard",
    copy: "Copy",
    regenerate: "Regenerate",
    stopGenerating: "Stop generating",
    welcomeTitle: "How can I help you today?",
    welcomeHint: "Ask me anything - I'm here to help with your questions",
    suggestions: [
      "Explain how to use this system",
      "Help me write a document",
      "What features are available?",
    ],
    orgSuggestions: [
      "Show me the organization structure",
      "How many employees are there?",
      "List all positions",
    ],
    orgMention: "Org Data",
    orgMentionHint: "Type @ to reference org data",
    orgBadge: "Org Data",
    tutorialButton: "Tutorial",
    tutorialBadge: "Tutorial",
    tutorialSelect: "Select a tutorial document",
    tutorialNone: "No tutorial documents available",
    tutorialTokens: "tokens",
    tutorialSuggestions: "Suggested questions from tutorial:",
    tutorialPages: "pages",
    ragButton: "RAG",
    ragBadge: "RAG",
    ragMention: "RAG Documents",
    ragDocCount: "docs",
    ragManage: "My Documents",
    ragDialog: {
      title: "RAG Documents",
      description: "Upload your own documents for AI-powered search and Q&A",
      upload: "Upload",
      uploading: "Uploading...",
      noDocuments: "No documents uploaded yet",
      noDocumentsHint: "Upload .md (Markdown) files",
      deleteConfirm: "Are you sure you want to delete this document?",
      chunks: "chunks",
      uploadSuccess: "Document uploaded successfully",
      deleteSuccess: "Document deleted successfully",
      uploadError: "Failed to upload document",
      deleteError: "Failed to delete document",
      close: "Close",
      dropHere: "Drop .md files here or click to select",
      dropActive: "Drop file here...",
      pasteText: "Paste Text",
      pasteTextTitle: "Paste Text as Document",
      filenameLabel: "Filename",
      filenamePlaceholder: "e.g. meeting-notes",
      textLabel: "Text Content",
      textPlaceholder: "Paste your markdown text here...",
      invalidFileType: "Only .md (Markdown) files are supported",
      submit: "Submit",
      manageTab: "Manage",
      browseTab: "Browse",
      chunkLabel: "Chunk",
      chunkChars: "chars",
      overlapLabel: "char overlap",
      pipelineTitle: "How RAG Works",
      pipelineStep1: "Document",
      pipelineStep2: "Chunk splitting",
      pipelineStep3: "Vectorization",
      pipelineStep4: "Similarity search",
      browseHint: "Each chunk is vectorized independently and searched by similarity to your question",
      noChunks: "No chunks available",
      loadingChunks: "Loading chunks...",
      sharedBadge: "Shared",
      personalBadge: "Personal",
      selectDocument: "Select a document to browse its chunks",
    },
    // Tutorial panel labels
    tutorialPanel: {
      title: "Document Analysis",
      pdfTab: "Original PDF",
      textTab: "Extracted Text",
      close: "Close panel",
      loading: "Loading...",
      textDescription: "AI uses this text as reference for answers",
      copyText: "Copy",
      copiedText: "Copied",
      pdfLoadError: "PDF preview is not available. Download the file to view.",
      openPanel: "Show panel",
    },
    // RAG Insight Panel
    ragInsight: {
      toggle: "View RAG search process",
      toggleClose: "Hide RAG search process",
      stepQuery: "Query",
      stepResults: "Vector search results",
      stepContext: "Context injection",
      hits: "hits",
      chunk: "Chunk",
      searchTime: "ms",
      contextExplain: "Top results added to AI prompt",
      contextExplain2: "AI generates answer using this information",
      tip: "The more specific your question, the higher the relevance scores",
    },
    // Stats labels
    stats: {
      contextUsage: "Context Usage",
      inputTokens: "Input",
      outputTokens: "Output",
      totalTokens: "Total",
      tokensPerSecond: "Speed",
      contextWindow: "Context Window",
      remaining: "Remaining",
      tokensUnit: "tokens",
      tpsUnit: "tok/s",
    },
  },
  ja: {
    title: "AIチャット",
    subtitle: "AIアシスタントとチャット",
    placeholder: "メッセージを入力...",
    send: "送信",
    thinking: "AIが考えています...",
    newChat: "新しいチャット",
    clearChat: "チャットをクリア",
    clearConfirm: "チャットをクリアしてもよろしいですか？",
    emptyState: "AIとの会話を始めましょう",
    emptyStateHint: "下のテキストボックスにメッセージを入力してください",
    errorTitle: "エラー",
    errorMessage:
      "AIからの応答を取得できませんでした。もう一度お試しください。",
    aiDisabled: "AIが有効になっていません",
    aiDisabledHint: "システム環境 > AI設定でAIを有効にしてください",
    you: "あなた",
    ai: "AI",
    copied: "クリップボードにコピーしました",
    copy: "コピー",
    regenerate: "再生成",
    stopGenerating: "生成を停止",
    welcomeTitle: "今日は何をお手伝いしましょうか？",
    welcomeHint: "何でも聞いてください - ご質問にお答えします",
    suggestions: [
      "このシステムの使い方を教えて",
      "ドキュメントの作成を手伝って",
      "どんな機能がありますか？",
    ],
    orgSuggestions: [
      "組織構造を教えて",
      "社員は何人いますか？",
      "役職一覧を表示して",
    ],
    orgMention: "組織データ",
    orgMentionHint: "@を入力して組織データを参照",
    orgBadge: "組織データ",
    tutorialButton: "チュートリアル",
    tutorialBadge: "チュートリアル",
    tutorialSelect: "チュートリアルドキュメントを選択",
    tutorialNone: "チュートリアルドキュメントがありません",
    tutorialTokens: "トークン",
    tutorialSuggestions: "チュートリアルのおすすめ質問:",
    tutorialPages: "ページ",
    ragButton: "RAG",
    ragBadge: "RAG",
    ragMention: "RAGドキュメント",
    ragDocCount: "件",
    ragManage: "マイドキュメント",
    ragDialog: {
      title: "RAGドキュメント",
      description: "自分のドキュメントをアップロードしてAI検索・質問応答を体験できます",
      upload: "アップロード",
      uploading: "アップロード中...",
      noDocuments: "ドキュメントがありません",
      noDocumentsHint: ".md（マークダウン）ファイルをアップロードできます",
      deleteConfirm: "このドキュメントを削除してもよろしいですか？",
      chunks: "チャンク",
      uploadSuccess: "ドキュメントをアップロードしました",
      deleteSuccess: "ドキュメントを削除しました",
      uploadError: "ドキュメントのアップロードに失敗しました",
      deleteError: "ドキュメントの削除に失敗しました",
      close: "閉じる",
      dropHere: ".mdファイルをドラッグ＆ドロップまたはクリックして選択",
      dropActive: "ここにファイルをドロップ...",
      pasteText: "テキスト貼り付け",
      pasteTextTitle: "テキストをドキュメントとして登録",
      filenameLabel: "ファイル名",
      filenamePlaceholder: "例: 議事録メモ",
      textLabel: "テキスト内容",
      textPlaceholder: "マークダウンテキストを貼り付けてください...",
      invalidFileType: ".md（マークダウン）ファイルのみ対応しています",
      submit: "登録",
      manageTab: "管理",
      browseTab: "ブラウズ",
      chunkLabel: "チャンク",
      chunkChars: "文字",
      overlapLabel: "文字オーバーラップ",
      pipelineTitle: "RAGの仕組み",
      pipelineStep1: "ドキュメント",
      pipelineStep2: "チャンク分割",
      pipelineStep3: "ベクトル化",
      pipelineStep4: "類似度検索",
      browseHint: "各チャンクは独立してベクトル化され、質問との類似度で検索されます",
      noChunks: "チャンクがありません",
      loadingChunks: "チャンクを読み込み中...",
      sharedBadge: "共有",
      personalBadge: "個人",
      selectDocument: "ドキュメントを選択してチャンクを閲覧できます",
    },
    // Tutorial panel labels
    tutorialPanel: {
      title: "ドキュメント分析",
      pdfTab: "原本PDF",
      textTab: "抽出テキスト",
      close: "パネルを閉じる",
      loading: "読み込み中...",
      textDescription: "AIはこのテキストを参考にして回答します",
      copyText: "コピー",
      copiedText: "コピー済",
      pdfLoadError: "PDFプレビューを読み込めません。ファイルをダウンロードしてご確認ください。",
      openPanel: "パネルを表示",
    },
    // RAG Insight Panel
    ragInsight: {
      toggle: "RAG検索プロセスを見る",
      toggleClose: "RAG検索プロセスを閉じる",
      stepQuery: "クエリ",
      stepResults: "ベクトル検索結果",
      stepContext: "コンテキスト注入",
      hits: "件ヒット",
      chunk: "チャンク",
      searchTime: "ms",
      contextExplain: "上位の結果をAIのプロンプトに追加",
      contextExplain2: "AIがこの情報を参考に回答を生成",
      tip: "質問が具体的なほど関連度が上がります",
    },
    // Stats labels
    stats: {
      contextUsage: "コンテキスト使用量",
      inputTokens: "入力",
      outputTokens: "出力",
      totalTokens: "合計",
      tokensPerSecond: "速度",
      contextWindow: "コンテキストウィンドウ",
      remaining: "残り",
      tokensUnit: "トークン",
      tpsUnit: "トークン/秒",
    },
  },
};

export type AIChatTranslations = typeof aiChatTranslations.en;
