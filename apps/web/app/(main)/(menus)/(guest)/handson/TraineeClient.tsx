...
import { fetchActiveSessions } from "@/components/handson/api";
import { handsonTranslations } from "@/components/handson/translations";
import type { Language, AvailableSession } from "@/components/handson/types";
...
export default function TraineeClient({ language, userId, userName }: Props) {
  const tc = handsonTranslations[language].common;
  const t = handsonTranslations[language].client;
  ...
  if (selectedSession) {
    return (
      <TraineeView
        ...
        onBack={availableSessions.length > 1 ? () => setSelectedSession(null) : undefined}
      />
    );
  }
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {session.participantCount}/{session.maxSeats} {tc.seats}
                    </div>
                  </div>
