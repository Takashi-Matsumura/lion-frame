...
export function RagDocumentsManager({
  ...
  const handleUpload = async (file: File) => {
    if (!file.name.toLowerCase().endsWith(".md") && !file.name.toLowerCase().endsWith(".markdown")) {
      alert(t("Only .md (Markdown) files are supported", ".md（マークダウン）ファイルのみ対応しています"));
      return;
    }
  ...
  const handleTextSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/rag-documents/upload-text", {
        method: "POST",
  ...
  const uploadZone = (compact?: boolean) => (
    ...
    >
      {uploading ? (
        <Loader2 className={`text-primary animate-spin mb-2 ${compact ? "w-5 h-5" : "w-8 h-8"}`} />
      ) : (
        <Upload className={`text-muted-foreground/50 mb-2 ${compact ? "w-5 h-5" : "w-8 h-8"}`} />
      )}
      <p className={`font-medium text-muted-foreground ${compact ? "text-xs" : "text-sm"}`}>
        {uploading
          ? t("Uploading...", "アップロード中...")
          : isDragOver
            ? t("Drop file here...", "ここにドロップ...")
            : t("Drop .md file", ".mdファイルをドロップ")}
      </p>
      {!uploading && (
        <p className={`text-muted-foreground/70 mt-1 ${compact ? "text-[10px]" : "text-xs"}`}>
          {t("or click to select", "またはクリックして選択")}
        </p>
      )}
    </div>
  );

  const pasteZone = (compact?: boolean) => (
    <div className={`border-2 border-dashed border-border rounded-lg ${compact ? "p-4" : "p-4"} flex flex-col`}>
      <div className="flex items-center gap-1.5 mb-2">
        <ClipboardPaste className={`text-muted-foreground/50 ${compact ? "w-4 h-4" : "w-5 h-5"}`} />
        <span className={`font-medium text-muted-foreground ${compact ? "text-xs" : "text-sm"}`}>
          {t("Paste text", "テキスト貼り付け")}
        </span>
      </div>
      <Input
        value={pasteFilename}
        onChange={(e) => setPasteFilename(e.target.value)}
        placeholder={t("Filename", "ファイル名")}
        className={`mb-2 ${compact ? "h-7 text-xs" : "h-8 text-sm"}`}
      />
      <Textarea
        value={pasteText}
        onChange={(e) => setPasteText(e.target.value)}
        placeholder={t("Paste markdown text...", "マークダウンテキストを貼り付け...")}
      ...
      {!loading && backendAvailable && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            {uploadZone(documents.length > 0)}
            {pasteZone(documents.length > 0)}
          </div>
          ...
          {documents.length === 0 ? (
            <div className="text-center py-2">
              <p className="text-xs text-muted-foreground">
                {t("No documents uploaded yet", "ドキュメントがまだありません")}
              </p>
            </div>
          ) : (
