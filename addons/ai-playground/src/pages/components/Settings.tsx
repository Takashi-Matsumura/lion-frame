...
import { LLMConfig, PROVIDER_PRESETS, LLMProviderType, SystemPrompts, SearchConfig, SearchProviderType, RAGConfig } from '../../types';
...
export function Settings({
  ...
  const handleRagToggle = async () => {
    ...
    try {
      const response = await fetch(`/api/ai-playground/rag?ragBaseUrl=${encodeURIComponent(ragConfig.baseUrl)}`);
  ...
  const handleConnectionTest = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai-playground/llm/test', {
        method: 'POST',
        headers: {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/ai-playground/rag/documents?ragBaseUrl=${encodeURIComponent(ragConfig.baseUrl.trim())}`);
      const data = await response.json();
      if (data.documents) {
  ...
  const fetchDocContent = useCallback(async (filename: string) => {
    ...
    try {
      const response = await fetch(`/api/ai-playground/rag/documents/${encodeURIComponent(filename)}?ragBaseUrl=${encodeURIComponent(ragConfig.baseUrl.trim())}`);
      const data = await response.json();
      if (data.chunks) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('/api/ai-playground/rag/documents', {
        method: 'POST',
  ...
  const handleDocSave = async () => {
    ...
    try {
      ...
      const deleteResponse = await fetch(
        `/api/ai-playground/rag/documents/${encodeURIComponent(selectedDoc)}?ragBaseUrl=${encodeURIComponent(ragConfig.baseUrl.trim())}`,
        { method: 'DELETE' }
      );
      ...
      // 2. 新しい内容で再登録
      const uploadResponse = await fetch('/api/ai-playground/rag/documents', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* モーダルコンテンツ */}
      <div className="relative bg-card rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[85vh] flex flex-col">
        {/* ヘッダー */}
            ...
            <div className="space-y-4">
              ...
              <div>
                <button
                  ...
                  className="w-full py-2 px-4 bg-muted hover:bg-muted disabled:opacity-50 text-foreground font-medium rounded-md transition-colors disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {connectionTesting ? (
                    <>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul className="space-y-0.5">
                      ...
                      <li>Ollama: <code className="bg-muted px-1 rounded">ollama serve</code></li>
                      <li>llama.cpp: <code className="bg-muted px-1 rounded">./server -m model.gguf</code></li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-sm border border-input rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring h-64 resize-none font-mono"
                  ...
                  <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-50 ${
                        ragEnabled ? 'bg-blue-500' : 'bg-muted'
                      }`}
                    >
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const response = await fetch(`/api/ai-playground/rag?ragBaseUrl=${encodeURIComponent(url)}`);
                          const data = await response.json();
                          if (data.status === 'healthy') {
                            setRagConnectionError(null);
              ...
              {ragSubTab === 'documents' && (
                <div className="space-y-4">
                  <div className="flex gap-4 h-[350px]">
                    {/* ドキュメント一覧 */}
                    <div className="w-1/3 border border-input rounded-lg overflow-hidden flex flex-col">
                      <div className="px-3 py-2 bg-muted text-xs font-medium text-muted-foreground border-b border-input">
                        ドキュメント一覧
                      </div>
                      <div className="flex-1 overflow-y-auto">
                        {documentsLoading ? (
                          ...
                          <div className="p-3 text-sm text-muted-foreground">ドキュメントがありません</div>
                        ) : (
                          documents.map((doc) => (
                            <div
                              key={doc.filename}
                              className={`px-3 py-2 text-sm cursor-pointer border-b border-border flex items-center justify-between group ${
                            ...
                            >
                              <div className="truncate flex-1">
                                ...
                                <div className="text-xs text-muted-foreground">{doc.chunk_count} chunks</div>
                              </div>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                    ...
                    {/* ドキュメント内容 */}
                    <div className="flex-1 border border-input rounded-lg overflow-hidden flex flex-col">
                      <div className="px-3 py-2 bg-muted text-xs font-medium text-muted-foreground border-b border-input flex items-center justify-between">
                        ...
                        {docContentLoading ? (
                          ...
                          docEditing ? (
                            <textarea
                              ...
                              disabled={docSaving}
                              className="flex-1 text-sm text-foreground font-mono bg-background border border-input rounded-md p-2 resize-none focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-50"
                            />
                          ) : (
                      ...
                      {docEditing && selectedDoc && (
                        <div className="px-3 py-2 border-t border-input space-y-2">
                          {docSaving && (
                          ...
                          <div className="flex justify-end">
                            <button
                              ...
                              className="px-3 py-1.5 bg-primary hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground text-primary-foreground text-xs font-medium rounded-md transition-colors disabled:cursor-not-allowed flex items-center gap-1.5"
                            >
                              {docSaving ? (
                                <>
                                  <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto space-y-4 pr-1">
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                      ...
                      <textarea
                        ...
                        className="w-full px-3 py-2 text-sm border border-input rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring flex-1 min-h-[120px] resize-none font-mono"
        ...
        {/* フッター */}
        <div className="p-4 border-t border-input flex justify-end gap-3">
          {activeTab === 'rag' && ragSubTab === 'upload' ? (
            <>
              <button
                onClick={onClose}
                className="px-4 py-2 bg-muted hover:bg-muted text-foreground text-sm font-medium rounded-md transition-colors"
              >
                閉じる
              </button>
              <button
                ...
                className="px-4 py-2 bg-primary hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground text-primary-foreground text-sm font-medium rounded-md transition-colors disabled:cursor-not-allowed flex items-center gap-2"
              >
                {uploading ? (
                  <>
                    <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
