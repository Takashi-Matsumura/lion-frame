import type { AddonModuleDefinition } from "@lionframe/module-types";

/**
 * Sample Hello モジュール定義
 *
 * アイコンは SVG パス文字列で定義する（ReactNode ではない）。
 * フレームワーク側で自動的に ReactNode に変換される。
 */
export const sampleHelloModule: AddonModuleDefinition = {
  id: "sample-hello",
  name: "Sample Hello",
  nameJa: "サンプル Hello",
  description: "A sample addon module demonstrating external module integration",
  descriptionJa: "外部モジュール連携のサンプルアドオン",
  // Heroicons: rocket-launch（ロケット＝サンプルアドオン）
  iconPath:
    "M15.59 14.37a6 6 0 01-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 006.16-12.12A14.98 14.98 0 009.631 8.41m5.96 5.96a14.926 14.926 0 01-5.841 2.58m-.119-8.54a6 6 0 00-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 00-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 01-2.448-2.448 14.9 14.9 0 01.06-.312m-2.24 2.39a4.493 4.493 0 00-1.757 4.306 4.493 4.493 0 004.306-1.758M16.5 9a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z",
  enabled: true,
  order: 90,
  menus: [
    {
      id: "sample-hello",
      moduleId: "sample-hello",
      name: "Hello",
      nameJa: "Hello",
      path: "/sample-hello",
      menuGroup: "user",
      requiredRoles: ["USER", "MANAGER", "EXECUTIVE", "ADMIN"],
      enabled: true,
      order: 90,
      description: "Sample hello page from external addon",
      descriptionJa: "外部アドオンのサンプルページ",
      isImplemented: true,
    },
  ],
};
