...
| 機能 | 実装 | ファイル |
|------|------|---------|
| 認証 | NextAuth.js v5 | `auth.ts`, `auth.config.ts` |
| LDAP認証 | OpenLDAP + Legacy LDAP | `lib/ldap/` |
| OAuth | Google / GitHub | `auth.config.ts` |
| 二要素認証 | TOTP | `app/api/user/two-factor/` |
...
| セキュリティヘッダ | X-Content-Type-Options等 | `middleware.ts` |
...
| パッケージ | 問題 | 対応 |
|-----------|------|------|
| xlsx | 未修正の脆弱性あり | 将来的に代替ライブラリに移行検討 |
