...
| カスタムクレーム | `lion:role`（USER/MANAGER/EXECUTIVE/ADMIN）／ `lion:mfa_used`（MFA 済みフラグ: パスキー認証時は true） |
...
認証オプションは `userVerification: "required"` / `residentKey: "required"` / `attestation: "none"` で固定。パスキーは所持（デバイス）＋ 生体/PIN で多要素を内包するため、**パスキーを第一の認証手段として推奨**しています。
...
- 最後の 1 件削除ガード: パスワード無しの状態で最後のパスキーを削除しようとするとサーバ側で 409 を返却（アカウントロックアウト防止）
- リカバリパス: 全デバイス紛失時はパスワードでログインして再登録。管理者は `/admin` の **複数 ADMIN 運用** または定期的なバックアップパスキー登録を推奨
