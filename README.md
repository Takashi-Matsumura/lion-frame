...
![モジュール管理画面](docs/images/module-management.png)
...
メインアプリとは独立した専用画面（`/kiosk/`）を提供するモジュールです。認証不要のキオスク端末向け画面と、メインアプリ内の管理UI（バックオフィスセクション）の二重構造を持ちます。

| モジュール | 説明 | キオスク画面 | 管理メニュー |
|-----------|------|------------|------------|
| **event-attendance** | NFCカードによるイベント出欠管理 | `/kiosk/events/[token]` | キオスク管理（バックオフィス） |

キオスクモジュールは `lib/kiosk-modules/` に配置します。モジュール定義で `kiosk: { basePath, requiresNfc }` を指定することで、モジュール管理画面では「Kiosk」カテゴリとして表示されます。
