...
LionFrame は社内の他アプリから OpenID Connect (OIDC) 経由で認証・RBAC を再利用できる **OIDC Provider (OP)** として機能する予定です（[Issue #6](https://github.com/Takashi-Matsumura/lion-frame/issues/6)）。

**設計方針:** **社内 LAN 環境限定の利用** を前提とし、暗号処理のみ [`jose`](https://github.com/panva/jose) ライブラリに委譲、各エンドポイントは Next.js App Router で自前実装する方式（Bパターン）を採用しています。外部公開や OIDC 認定取得の予定がないため、統合ライブラリ（`node-oidc-provider`）や外部 OP（Ory Hydra / Keycloak）は採用していません。

**選定理由・各選択肢の比較・将来拡張の余地** は [OIDC_PROVIDER.md](docs/OIDC_PROVIDER.md) に記録しています。将来前提が変わる場合（外部公開など）は必ず参照してください。
